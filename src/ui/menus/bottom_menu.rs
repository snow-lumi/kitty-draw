...
fn bottom_menu_fn(ui: &mut Ui, kitty: &mut Kitty ) {
    ui.horizontal(|ui| {
        // pointer coordinates
        ui.label("x: ");
        let meow = ui.add_sized((70.0,20.0), egui::TextEdit::singleline(&mut kitty.x_string));
        ui.label("y: ");
...
pub fn bottom_menu(kitty: &mut Kitty) -> impl FnMut(&mut Ui) {
    |ui| {bottom_menu_fn(ui, kitty);}
}